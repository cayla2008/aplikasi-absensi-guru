    ...
    <div class="w-full px-6 py-6 mx-auto">
        <div class="relative flex flex-col min-w-0 break-words bg-white border-0 shadow-xl rounded-2xl bg-clip-border">
            <div class="p-6 pb-0 mb-0 border-b-0 border-b-solid border-gray-200 rounded-t-2xl">
                <h6 class="text-lg font-bold text-slate-700">Edit Data Guru</h6>
            </div>
                ...
                <form action="{{ route('guru.update', $guru->id) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="jenis_kelamin" id="jenis_kelamin"
                            ...
                            <option value="lelaki" {{ $guru->jenis_kelamin == 'lelaki' ? 'selected' : '' }}>Lelaki</option>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="alamat" id="alamat"
                            class="focus:shadow-primary-outline text-sm leading-5.6 ease block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white px-3 py-2 font-normal text-gray-700 outline-none transition-all placeholder:text-gray-500 focus:border-blue-500 focus:outline-none"
                            placeholder="Masukkan Alamat">{{ old('alamat', $guru->alamat) }}</textarea>
                    </div>
                    ...
                    <div class="mb-4">
                        <label for="no_tlp" class="inline-block mb-2 ml-1 font-bold text-xs text-slate-700">Nomor
                            HP</label>
                        <input type="text" name="no_tlp" id="no_tlp" value="{{ old('no_tlp', $guru->no_tlp) }}"
                            class="focus:shadow-primary-outline text-sm leading-5.6 ease block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white px-3 py-2 font-normal text-gray-700 outline-none transition-all placeholder:text-gray-500 focus:border-blue-500 focus:outline-none"
                    ...
                    <div class="mb-4">
                        ...
                        <select name="email" id="email"
                            ...
                            <option value="" disabled>Pilih Email</option>

                            @forelse ($users as $user)
                                <option value="{{ $user->email }}" {{ $user->email == $guru->email ? 'selected' : '' }}>
                                    {{ $user->email }}
                                </option>
                            @empty
                                <option value="">Tidak ada email tersedia</option>
                            @endforelse
                        </select>
                    </div>
