    ...
    <div class="w-full px-6 py-6 mx-auto">
        <div class="relative flex flex-col min-w-0 break-words bg-white border-0 shadow-xl rounded-2xl bg-clip-border">
            <div class="p-6 pb-0 mb-0 border-b-0 border-b-solid border-gray-200 rounded-t-2xl">
                <h6 class="text-lg font-bold text-slate-700">Edit Data Kelas</h6>
            </div>
                ...
                <form action="{{ route('kelas.update', $kelas->id) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="nama_kelas" id="nama_kelas"
                            ...
                            <option value="X" {{ $kelas->nama_kelas == 'X' ? 'selected' : '' }}>Kelas X</option>
                            <option value="XI" {{ $kelas->nama_kelas == 'XI' ? 'selected' : '' }}>Kelas XI</option>
                            <option value="XII" {{ $kelas->nama_kelas == 'XII' ? 'selected' : '' }}>Kelas XII</option>
                        </select>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="jurusan" id="jurusan"
                            ...
                            <option value="RPL" {{ $kelas->jurusan == 'RPL' ? 'selected' : '' }}>Rekayasa Perangkat Lunak
                                (RPL)</option>
                            <option value="TKJ" {{ $kelas->jurusan == 'TKJ' ? 'selected' : '' }}>Teknik Komputer dan
                                Jaringan (TKJ)</option>
                            <option value="AKL" {{ $kelas->jurusan == 'AKL' ? 'selected' : '' }}>Akuntansi dan Keuangan
                                Lembaga (AKL)</option>
                            <option value="OTKP" {{ $kelas->jurusan == 'OTKP' ? 'selected' : '' }}>Otomatisasi dan Tata
                                Kelola Perkantoran (OTKP)</option>
                            <option value="BDP" {{ $kelas->jurusan == 'BDP' ? 'selected' : '' }}>Bisnis Daring dan
                                Pemasaran (BDP)</option>
                        </select>
                    </div>
